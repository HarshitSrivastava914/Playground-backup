...
export const PaymentMethodSelector: React.FC<PaymentMethodSelectorProps> = ({
  ...
  const handleAuthTypeToggle = (value: string) => {
  ...
  const handleProceed = () => {
    ...
    let summary = selectedMethod;

    if (selectedMethod === "emandate") {
      summary = `eMandate ${
        enableAuthType
          ? selectedAuthTypes.length > 0
            ? `- Auth Types: ${selectedAuthTypes.join(", ")}`
          ...
          {enableAuthType && (
            <div>
              ...
              <div className="flex gap-8 justify-center mb-6">
                {authTypes.map((auth) => {
                  const isSelected = selectedAuthTypes.includes(auth.value);
                  return (
                    <div
                      key={auth.value}
                      ...
                      onClick={() => handleAuthTypeToggle(auth.value)}
                    >
        ...
        <div className="mt-10 text-center border-t border-gray-700 pt-6">
          ...
          {selectedMethod === "emandate" && (
            <>
              ...
              {enableAuthType && selectedAuthTypes.length > 0 && (
                <p className="text-gray-300">
                  <strong>Selected Auth Types:</strong>{" "}
                  {selectedAuthTypes
                    .map((val) => authTypes.find((a) => a.value === val)?.label)
                    .join(", ")}
                </p>
              )}
            </>
